...
/*************start here******************/
#ifdef WJL_MC			// WJL_MC is short for WangJinLei_MultiCore
#define __USE_GNU
#include <pthread.h>
#endif
/*************end here***************************/
...
/********************starts here************************/
#ifdef WJL_MC
  static unsigned no_cores = 1;
  static unsigned no_wc = 1; /*working cors*/
  static unsigned queue_size = 1024;
  static unsigned main_que_size = 1024;
  static void *que_buf;
  static int next_core = -1;
  typedef struct		// the definiction for data item
  {
      void * data;
      bpf_u_int32 len;
  } Que_item;
  typedef struct       // the definition for fifo
  {
    unsigned rear, front, collect;
    Que_item *item;
  } Queue;
  static Queue * ques;
  static unsigned hash_def(void * data);	
  static void *mc_check_queue(void *);
  static unsigned (*hash_func)(void *) = hash_def;  // mkhash
  static void *mc_collector(void *);                // this function restores the parallel execute flow into sequential one
  static void do_nothing(u_char * data, int len);   // 
#endif
/********************starts here************************/
...
static void call_ip_frag_procs(void *data,bpf_u_int32 caplen)
{
  ...
  #ifdef WJL_MC
    unsigned hash;
    hash = hash_func(data) % no_wc;
  /*  printf("hash = %u\n", hash);*/
    if (((ques[hash].rear + 1) % queue_size) != ques[hash].collect)
    {
      ques[hash].item[ques[hash].rear].data = data;
      ques[hash].item[ques[hash].rear].len = caplen;
      ques[hash].rear = (ques[hash].rear + 1) % queue_size;
    }
  #else
...
#define DLT_LINUX_SLL   113
    ...
    /*Initialization*/
      ...
      /*decide how many working cores*/
      no_cores = sysconf(_SC_NPROCESSORS_CONF);
      if(no_cores == 1)
        no_wc = 1;
      else
        no_wc = no_cores - 1;
      if((ques = (Queue *) malloc(sizeof (Queue) * no_wc)) == NULL)
        return 0;
      if((que_buf = malloc(sizeof(Que_item) * no_wc * queue_size)) == NULL)
        return 0;
      for(i = 0; i < no_wc; i ++)
      {
        ques[i].rear = ques[i].front = ques[i].collect = 0;
        ques[i].item = ((Que_item *)que_buf) + i * queue_size;
      }
      hash_func = hash_def;
      if (no_cores == 1)
      {
        puts("You have only one cpu.");
        next_core = 0;
        pthread_create(&pid,NULL,mc_check_queue,NULL);
      }
      else
      {
        for(i = 1; i <= no_wc; i ++)
        {
          next_core = i;
          pthread_create(&pid,NULL,mc_check_queue,NULL);
          while(next_core >= 0)sleep(0);
        }
      }
      pthread_create(&pid,NULL,mc_collector,NULL);
    #endif
...
#ifdef WJL_MC
  unsigned hash_def(void * data)
  {
      /*
    static unsigned rewrite_here = 0;
    return rewrite_here ++;
*/
      int no_queue = sysconf(_SC_NPROCESSORS_CONF);

      struct ip* this_iphdr = (struct ip *)data;
      struct tcphdr* this_tcphdr = (struct tcphdr *)(data +4*this_iphdr->ip_hl); 
      unsigned int dst_ip = ntohl(this_iphdr->ip_dst.s_addr);
      unsigned int src_ip = ntohl(this_iphdr->ip_src.s_addr);

      unsigned short dst_port = this_tcphdr->th_dport;
      unsigned short src_port = this_tcphdr->th_sport;
     
      return (dst_ip + src_ip + dst_port + src_port)%no_queue + 1;

 }
//*************************************************************
// ip_frag_procs  : the global var
// ip_frag_procs --> user_reg_fun1 --> user_reg_fun2 --> ... --> gen_ip_frag_proc.

/*
________________________________________________________________________________________
|                                                                                       |        
|                --> user_reg_fun1 --> user_reg_fun2 --> ... -->                        |              
|              /                                                  \                     |
|                                                                                       |        
|core0 dispatch  ---> user_reg_fun1 --> user_reg_fun2 --> ...-->    gen_ip_frag_proc    |
|                                                                                       |
|              \                                                  /                     |
|                --> user_reg_fun1 --> user_reg_fun2 --> ... -->                        |
|                                                                                       |   
...
*/
  void *mc_check_queue(void *not_used)
  {
    int core_index;
    struct proc_node *pfunc;
    core_index = next_core;
    next_core = -1;
    ...
    sched_setaffinity(0,sizeof(mask),&mask);
    core_index = core_index > 0 ? core_index - 1 : core_index;
    while(1)
    {
        while(ques[core_index].rear == ques[core_index].front)sleep(0);
/*printf("[%d]collect=%u, front=%u, rear=%u\n",core_index,ques[core_index].collect,ques[core_index].front,ques[core_index].rear);*/
        for(pfunc = ip_frag_procs; pfunc; pfunc = pfunc->next)
        {
            pfunc->item(ques[core_index].item[ques[core_index].front].data,
                        ques[core_index].item[ques[core_index].front].len);
        }
        ques[core_index].front = (ques[core_index].front + 1) % queue_size;
    }
    return NULL;
  }

  // *****************************************************************
// restores into a sequential one
/*
_________________________________________________________________
                                                                 |        
  --> .core1.. -->    \                                          |              
                                                                 |
  --> .core2.. -->    --> gen_ip_frag_proc --> ip_defrg_stub()...|
                                                                 |             
  --> .core3.. -->    /                                          |
                                                                 |   
________________________________________________________________ |
*/
  void *mc_collector(void *not_used)
  {
    int i;
    cpu_set_t mask;
    CPU_ZERO(&mask);
    ...
    if (no_wc == 1)
    {
      while (1)
      {
        while (ques->front == ques->collect)sleep(0);
        gen_ip_frag_proc(ques->item[ques->collect].data,ques->item[ques->collect].len);
        ques->collect = (ques->collect + 1) % queue_size;
      }
    }
    else
    {
      while (1)
      {
        for(i = 0; i < no_wc; i ++)
        {
          if (ques[i].collect != ques[i].front)
          {
            gen_ip_frag_proc(ques[i].item[ques[i].collect].data,ques[i].item[ques[i].collect].len);
            ques[i].collect = (ques[i].collect + 1) % queue_size;
          }
        }
      }
    }
